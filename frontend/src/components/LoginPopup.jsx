  ...
  return (
    <div className='absolute h-full w-full bg-black/40 z-50 flexCenter'>
        <form onSubmit={onLogin} className='bg-white w-[366px] p-7 rounded-lg shadow-md'>
            ...
            <FaXmark className='medium-20 text-slate-900/70 cursor-pointer'
            onClick={()=> setShowLogin(false)}
            />
            </div>
            <div className='flex flex-col gap-4 my-6'>
                { state==="Sign Up" && (
                <input onChange={onChangeHandler} name='name' value={data.name} type="text" placeholder='Name' required className='border border-slate-900/20 p-2 pl-4 rounded-md outline-none' />)}
                <input onChange={onChangeHandler} name='email' value={data.email} type="email" placeholder='Email' required className='border border-slate-900/20 p-2 pl-4 rounded-md outline-none' />
                <input onChange={onChangeHandler} name='password' value={data.password} type="password" placeholder='Password' required className='border border-slate-900/20 p-2 pl-4 rounded-md outline-none' />
            </div>
